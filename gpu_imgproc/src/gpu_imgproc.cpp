...
GpuImgProc::GpuImgProc()
    :  Node("GpuImgProcNode") {
    RCLCPP_INFO(get_logger(), "Initializing node gpu_imgproc");
    ...
    std::string rect_impl = declare_parameter<std::string>("rect_impl");
    bool use_opencv_map_init = declare_parameter<bool>("use_opencv_map_init");
    ...
    if (0) {
        ...
        RCLCPP_INFO(get_logger(), "Using NPP implementation for rectification");
        rectifier_impl_ = Rectifier::Implementation::NPP;
#endif
#ifdef OPENCV_AVAILABLE
    } else if (rect_impl == "opencv_cpu") {
        RCLCPP_INFO(get_logger(), "Using OpenCV CPU implementation for rectification");
        rectifier_impl_ = Rectifier::Implementation::OpenCV_CPU;
#endif
#ifdef OPENCV_CUDA_AVAILABLE
    } else if (rect_impl == "opencv_gpu") {
        RCLCPP_INFO(get_logger(), "Using GPU OpenCV implementation for rectification");
        rectifier_impl_ = Rectifier::Implementation::OpenCV_GPU;
#endif
    } else {
        RCLCPP_INFO(get_logger(), "Available implementations: %s", available_impls.c_str());
        return;
    }

    if (use_opencv_map_init) {
        RCLCPP_INFO(get_logger(), "Using OpenCV map initialization");
        mapping_impl_ = Rectifier::MappingImpl::OpenCV;
    } else {
        RCLCPP_INFO(get_logger(), "Using Non-OpenCV map initialization");
...
#else
    RCLCPP_ERROR(get_logger(), "No JPEG compressor available");
    return;
#endif
...
GpuImgProc::~GpuImgProc() {
    RCLCPP_INFO(get_logger(), "Shutting down node gpu_imgproc");
}
        ...
        rectified_msg =
            std::async(std::launch::async, [this, msg]() {
                ...
                if (false) {
                    ...
                    RCLCPP_ERROR(get_logger(), "Invalid implementation");
                    return;
                }
...
void GpuImgProc::cameraInfoCallback(const CameraInfo::ConstPtr &msg) {
    ...
    if (msg->d.size() == 0 || msg->p.size() == 0) {
        RCLCPP_ERROR(get_logger(), "Camera info message does not contain distortion or projection matrix");
        return;
    }
...
#if NPP_AVAILABLE
            RCLCPP_INFO(get_logger(), "Initializing NPP rectifier");
            npp_rectifier_ = std::make_shared<Rectifier::NPPRectifier>(*msg, mapping_impl_, alpha_);
            if (npp_rectifier_) {
                RCLCPP_INFO(get_logger(), "Initialized NPP rectifier");
                rectifier_active_ = true;
            } else {
                RCLCPP_ERROR(get_logger(), "Failed to initialize NPP rectifier");
                return;
            }
            break;
#else
            RCLCPP_ERROR(get_logger(), "NPP not enabled");
            return;
#endif
        case Rectifier::Implementation::OpenCV_CPU:
#ifdef OPENCV_AVAILABLE
            RCLCPP_INFO(get_logger(), "Initializing OpenCV CPU rectifier");
            cv_cpu_rectifier_ = std::make_shared<Rectifier::OpenCVRectifierCPU>(*msg, mapping_impl_, alpha_);
            if (cv_cpu_rectifier_) {
                RCLCPP_INFO(get_logger(), "Initialized OpenCV CPU rectifier");
                rectifier_active_ = true;
            } else {
                RCLCPP_ERROR(get_logger(), "Failed to initialize OpenCV rectifier");
                return;
            }
            break;
#else
            RCLCPP_ERROR(get_logger(), "OpenCV not enabled");
            return;
#endif 
        case Rectifier::Implementation::OpenCV_GPU:
#ifdef OPENCV_CUDA_AVAILABLE
            RCLCPP_INFO(get_logger(), "Initializing OpenCV GPU rectifier");
            cv_gpu_rectifier_ = std::make_shared<Rectifier::OpenCVRectifierGPU>(*msg, mapping_impl_, alpha_);
            if (cv_gpu_rectifier_) {
                RCLCPP_INFO(get_logger(), "Initialized OpenCV GPU rectifier");
                rectifier_active_ = true;
            } else {
                RCLCPP_ERROR(get_logger(), "Failed to initialize OpenCV rectifier");
                return;
            ...
            break;
#else
            RCLCPP_ERROR(get_logger(), "OpenCV CUDA not enabled");
            return;
#endif
        default:
            RCLCPP_ERROR(get_logger(), "Invalid rectifier implementation");
            return;
    }
